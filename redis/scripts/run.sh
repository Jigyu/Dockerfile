...
if [ ! -f /.redis_configured ]; then
    ...
    for i in $(printenv | grep REDIS_); do
        echo $i | sed "s/REDIS_//" | sed "s/_/-/" | sed "s/=/ /" | sed "s/^[^ ]*/\L&\E/" >> /etc/redis_default.conf
    done

    echo "=> Using redis.conf:"
    cat /etc/redis_default.conf | grep -v "requirepass"

    touch /.redis_configured
fi

exec /usr/bin/redis-server /etc/redis_default.conf
